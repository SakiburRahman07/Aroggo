      ...
      <div className="mx-auto max-w-md rounded-[32px] border border-white/70 bg-white/90 p-8 shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
        ...
        <div className="mt-8">
          <PortalLoginForm
            ...
            activationRequired={params.activation === "required"}
            invalidQr={params.error === "invalid"}
            expiredQr={params.error === "expired"}
            revokedQr={params.error === "revoked"}
            accountError={params.error === "account"}
            unauthorizedQr={params.error === "unauthorized"}
          />
        </div>
      </div>
    </div>
